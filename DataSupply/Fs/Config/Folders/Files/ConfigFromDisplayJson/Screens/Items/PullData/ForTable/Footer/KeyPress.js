...
let LocalPullData = async ({ inEnterToServer, inUserPK, inValueToFind }) => {
    ...
    let LocalDataRowFound = LocalDataAsArray.find(element => element[LocalColumnNameToFind] === inValueToFind);
...
let LocalSwitch = async ({ inEnterToServer, inUserPK, inValueToFind }) => {
    let LocalReturnObject = { KTF: false, KReason: "" };
    ...
    switch (inEnterToServer.PickFrom) {
        case "Reports":
            ...
            break;
        case "Data":
            let LocalDataNeeded = await LocalPullData({ inEnterToServer, inUserPK, inValueToFind });
...
let EnterToServerSubTable = async ({ inJsonConfig, inItemConfig, inInsertKey, inUserPK, inPostData }) => {
    ...
    let LocalDataNeeded = await LocalSwitch({ inEnterToServer: LocalEnterToServer, inUserPK, inPostData });
...
let EnterToServerFromMainTable = async ({ inJsonConfig, inItemConfig, inUserPK, inPostData }) => {
    ...
    if (LocalEnterToServer === undefined) {
        ...
        let LocalDataNeeded = await LocalSwitch({
            ...
            inUserPK,
            inValueToFind: LocalValueToFind
        });
